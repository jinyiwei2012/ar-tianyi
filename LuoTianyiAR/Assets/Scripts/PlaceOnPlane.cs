...
// 综合说明（debut × phase3 合并）：
// - 层级采用 debut 的 Pose Root 设计：Anchor -> LuoTianyi Placement Root（世界位姿=命中点）-> Live2D；
//   billboard 挂在 Pose Root 上，旋转始终围绕真正落点。脚底对齐用腿 Drawable 精确落地，
//   由 PlaceFeetOnAnchor 在模型子对象上做枢轴校正，根节点保持与 Anchor/定位卡对齐。
// - 交互保留双方能力：中心准星/点选放置、定位卡优先首放、拖动阈值、位置锁定（锁定后点击=视线追踪）、
//   Phase 2 点击走路（拖动接管会打断走路）、双指缩放、Nudge/表情 API。
// - Anchor 创建使用 debut 的异步 TryAddAnchorAsync + 请求版本管理，避免首帧 Transform 未同步。
...
public class PlaceOnPlane : MonoBehaviour
{
    ...
    [Header("交互模式")]
    [Tooltip("true=中心准星模式：首次放置放到屏幕中心准星（或定位卡）指向处，点击仅作为确认；false=点选模式：模型放到手指点击处。")]
    ...
    // Phase 2 移动：模型放置后挂载到 Pose Root 上，由点击走路 / 拖动接管。
    ...
    private void OnFingerDown(Finger finger)
    {
        ...
        // 交互分派（按优先级）：
        // 1) 未放置 → 首次放置：定位卡 > 中心准星/点选，点击仅作为“确认”；
        // 2) 已锁定位置 → 点击/拖动变为视线追踪焦点，模型不再移动；
        // 3) 已放置就绪 → 点击目标点让洛天依走过去（Phase 2）。
        ...
        else if (spawnedModel != null && modelReady && luoMovement != null)
        {
            // 中心准星模式下行走目标同样取屏幕中心，保持“看哪放哪/走哪”一致。
            Vector2 requestPoint = centerCrosshairMode
                ? GetScreenCenter()
                : finger.screenPosition;
    ...
    private void OnFingerMove(Finger finger)
    {
        ...
        // 手指移动超过拖动阈值才进入拖动，避免普通点击/微移重建 Anchor 偷换位置。
    ...
    private async void TryPlaceAtScreenPosition(Vector2 screenPosition, bool showFeedback)
    {
        ...
        if (spawnedModel == null)
        {
            ...
            // Phase 2：程序化动画挂到模型（呼吸/头部微摆/走路律动）；
            // 移动组件挂到稳定根节点，注入依赖。眨眼/表情/物理由 Live2DModelFeatures 的
            // SDK Framework 组件负责，双方参数分工见 LuoMotionAnimation 头注释。
            var motionAnimation = spawnedModel.GetComponent<LuoMotionAnimation>() ??
                                  spawnedModel.AddComponent<LuoMotionAnimation>();
            luoMovement = modelPoseRoot.AddComponent<LuoMovement>();
            luoMovement.Initialize(raycastManager, billboard, motionAnimation);

            // Phase 3：遮挡参照立方体（普通 URP 几何），随 Pose Root 一起对齐地面。
    ...
    private void SpawnOcclusionTestCube()
    {
        ...
        cube.transform.SetParent(modelPoseRoot.transform, false);
    ...
    public string GetDebugSnapshot()
    {
        ...
        return
            ...
            $"anchor={anchorState}, targetHeight={targetHeightMeters:F3}m, mode={(centerCrosshairMode ? "center" : "tap")}, positionLock={(positionLocked ? "locked" : "adjustable")}\n" +
