...
public sealed class AutoHarmonizationController : MonoBehaviour
{
    ...
    [SerializeField, Range(0f, 1f)] private float correctionStrength = 0.72f;
    ...
    private void EvaluatePaperLighting(Vector3 frontNormal, out Color multiply, out Color screen)
    {
        ...
        else if (hasSphericalHarmonics)
        {
            ...
            estimationSource = "environmental-hdr";
            if (hasMainDirection)
            {
        ...
        {
            estimationSource = "ambient-intensity";
        ...
        Color target = new(
            Mathf.Clamp(brightnessFactor * tint.r, effectiveMinimumBrightness, maximumBrightness),
            Mathf.Clamp(brightnessFactor * tint.g, effectiveMinimumBrightness, maximumBrightness),
            Mathf.Clamp(brightnessFactor * tint.b, effectiveMinimumBrightness, maximumBrightness),
            1f);
        target = Color.Lerp(Color.white, target, correctionStrength);
